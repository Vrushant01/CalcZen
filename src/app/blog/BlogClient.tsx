...
interface BlogClientProps {
  initialBlogs: Blog[];
  initialTotal: number;
  trendingCalculatorSlugs: string[];
}
...
export default function BlogClient({
  initialBlogs,
  initialTotal,
  trendingCalculatorSlugs,
}: BlogClientProps) {
  const trendingCalculators = useMemo(() => {
    return (trendingCalculatorSlugs || [])
  ...
  const [totalPages, setTotalPages] = useState(Math.ceil(initialTotal / 9) || 1);

  // We bypass the first client-side query load to avoid double-fetching and layout shift
            ...
            {loading ? (
              ...
              <>
                ...
                {page === 1 && !search && featuredBlog && (
                  ...
                  >
                    <Link
                      to={`/blog/${(featuredBlog.category || "finance").toLowerCase()}/${featuredBlog.slug}`}
                    ...
                    <AnimatePresence mode="popLayout">
                      {blogs
                        ...
                        .map((b, i) => (
                          ...
                          >
                            <Link
                              to={`/blog/${(b.category || "finance").toLowerCase()}/${b.slug}`}
              ...
              <div className="rounded-2xl border border-border bg-card/25 p-5 min-w-0">
                ...
                <div className="space-y-4">
                  {popularBlogs.map((pop) => (
                    <Link
                      key={pop._id}
                      to={`/blog/${(pop.category || "finance").toLowerCase()}/${pop.slug}`}
