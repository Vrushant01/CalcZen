...
export function CompoundInterestCalculator() {
  ...
  const { series, total, contributed, earned } = useMemo(() => {
    ...
    for (let m = 1; m <= n; m++) {
      ...
      contrib += calcMonthly;
      if (m % 12 === 0)
        series.push({ year: m / 12, balance: Math.round(bal), contributed: Math.round(contrib), earned: Math.round(bal - contrib) });
    }
  ...
  const cagr = calcYears > 0 ? ((Math.pow(total / Math.max(contributed, 1), 1 / calcYears) - 1) * 100).toFixed(2) : "0";
  ...
  const comparisonRows = rateOptions.map((r) => {
    ...
    let bal = calcPrincipal;
    let c = calcPrincipal;
    for (let m = 1; m <= n; m++) { bal = bal * (1 + mr) + calcMonthly; c += calcMonthly; }
  ...
  const isButtonDisabled = principal === "" || monthly === "" || rate === "" || years === ""
    || Number(principal) < 0 || Number(monthly) < 0 || Number(rate) <= 0 || Number(years) <= 0;
  ...
  const handleReset = () => {
    setPrincipal(10000); setMonthly(500); setRate(8); setYears(20);
    setCalcPrincipal(10000); setCalcMonthly(500); setCalcRate(8); setCalcYears(20);
    resetCalculated();
  };
  ...
  return (
    <CalculatorPageLayout
      ...
      faqs={[
        { q: "What is compound interest?", a: "Compound interest is the addition of interest to the principal sum of a deposit or loan, where interest earns interest on itself. This compounding effect causes your investments to grow exponentially over time, especially when compared to simple interest. It is a vital cornerstone of long-term financial planning and wealth creation." },
        { q: "How does compound frequency affect investment growth?", a: "Compound frequency refers to how often accumulated interest is calculated and added to the principal balance. The more frequently interest compounds (e.g., daily or monthly instead of annually), the faster your wealth accumulates. You can calculate other borrowing scenarios with our <a href=\"/calculator/loan-emi-calculator\" class=\"text-primary hover:underline\">Loan EMI Calculator</a> to compare debt amortization schedules." },
        { q: "What is the difference between simple and compound interest?", a: "Simple interest is calculated solely on the initial principal amount invested. Compound interest is calculated on the initial principal plus all interest accumulated from previous periods. Over long horizons, this compounding difference creates a massive divergence in total savings growth, making compounding far superior for your long-term wealth building and retirement security." },
        { q: "How does inflation affect my compound returns?", a: "Inflation erodes the purchasing power of your money over time, meaning a dollar today buys less in the future. When evaluating long-term investment projections, you must subtract the inflation rate from your nominal yield to find the real rate of return and avoid overestimating the future purchasing power of your accumulated savings." },
        { q: "Can I calculate future value with monthly contributions?", a: "Yes, our calculator lets you model monthly or annual contributions alongside your initial principal. Regular contributions accelerate the compounding process, as interest is immediately earned on the new deposits. You can check percentage-based budget allocations with our <a href=\"/calculator/percentage-calculator\" class=\"text-primary hover:underline\">Percentage Calculator</a> to optimize your monthly savings goals and track regular investment growth." },
        { q: "What is the Rule of 72?", a: "The Rule of 72 is a quick, handy mental formula used to estimate how many years it will take for an investment to double at a fixed annual rate of interest. Simply divide 72 by your annual interest rate. For example, at a 6% return, your money doubles in about 12 years." },
        { q: "Why is starting early so important for compound growth?", a: "Starting early is critical because compounding interest relies heavily on time. In the initial years, growth is slow, but in later decades, the exponential curve bends sharply upward. A delay of just five years early in life can cut your final retirement nest egg in half, showing how costly procrastination is." },
        { q: "How does compound interest relate to retirement planning?", a: "Retirement planning relies on compounding interest to build a nest egg large enough to support you without working. By consistently saving over your career, your contributions build a self-sustaining asset. Estimate your retirement targets and contribution matches with our dedicated <a href=\"/calculator/retirement-calculator\" class=\"text-primary hover:underline\">Retirement Calculator</a> to secure your long-term future." }
      ]}
    ...
    >
      ...
      <div className="flex flex-col gap-6">
        <div className="calc-input-column">
          ...
          <MoneyField label="Monthly contribution" value={monthly} onChange={(v) => setMonthly(v)} />
          <div className="calc-field-grid-2">
            <NumField label="Annual return" value={rate} onChange={(v) => setRate(v)} onKeyDown={handleKeyDown} suffix="%" step={0.1} />
            <NumField label="Years" value={years} onChange={(v) => setYears(v)} onKeyDown={handleKeyDown} suffix="yr" />
          </div>
          ...
          >
            ...
            <DashboardSection title="Key Metrics">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <StatCard index={0} label="Total Contributed" value={format(contributed)} accent="blue" />
                <StatCard index={1} label="Interest Earned" value={format(earned)} accent="green"
                  subValue={`${((earned / total) * 100).toFixed(0)}% of final balance`} />
                <StatCard index={2} label="Growth Multiplier" value={`${multiplier}×`} accent="purple" />
                <StatCard index={3} label="Effective CAGR" value={`${cagr}%`} accent="cyan" />
                <StatCard index={4} label="Money Doubles Every" value={`${rule72} yrs`} accent="amber" subValue="Rule of 72" />
                <StatCard index={5} label="5 Yrs Earlier = +" value={fiveYearsEarlier > 0 ? format(fiveYearsEarlier) : "N/A"} accent="green" />
              </div>
            </DashboardSection>

            {/* Chart */}
            <DashboardSection title="Growth Projection">
              <div id="compound-chart" className="rounded-xl border border-border/60 bg-card p-4 shadow-soft">
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={series} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      ...
                      <Area type="monotone" dataKey="balance" name="Balance" stroke="var(--color-chart-1)"
                        strokeWidth={2.5} fill="url(#balGrad)" dot={false} />
                      <Area type="monotone" dataKey="contributed" name="Contributed" stroke="var(--color-chart-2)"
                        strokeWidth={2} fill="url(#contribGrad)" dot={false} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              </div>
            ...
            <DashboardSection title="Smart Insights">
              <div className="flex flex-col gap-2">
                <InsightCard index={0} tone="success"
            ...
            <DashboardSection title="Recommendations">
              <RecommendationList items={[
                { title: "Automate monthly contributions", description: `Your ${format(calcMonthly)}/month contribution is the engine of your wealth. Automate it so you never miss a month — consistency beats timing every time.` },
                { title: "Consider tax-advantaged accounts", description: "Placing investments in 401(k), IRA, or similar tax-sheltered accounts lets compounding work on money that would otherwise go to taxes." },
                { title: "Increase contribution with income", description: "A 1% salary increase put into investments adds significantly over decades. Revisit your contribution rate every year or when your income rises." },
              ]} />
            </DashboardSection>
...
function NumField({ label, value, onChange, onKeyDown, suffix, step = 1 }: {
