...
export function LoanEMICalculator() {
  ...
  const { emi, totalInterest, totalPay } = useMemo(() => {
    ...
    const n = calcMonths;
    const emi =
      r === 0 ? calcAmount / n : (calcAmount * r * Math.pow(1 + r, n)) / (Math.pow(1 + r, n) - 1);
  ...
  const isButtonDisabled =
    !amount || !rate || !months || Number(amount) <= 0 || Number(rate) <= 0 || Number(months) <= 0;
          ...
          >
            ...
            <DashboardSection title="Smart Insights">
              <div className="flex flex-col gap-2">
                ...
                <InsightCard
                  index={1}
                  tone={interestPctNum > 40 ? "warning" : "success"}
            ...
            <DashboardSection title="Recommendations">
              <RecommendationList
                items={[
                  {
                    title: "Make EMI-to-income check",
                    description: `Your EMI should ideally not exceed 40% of your monthly take-home income. At ${format(emi)}/month, ensure this fits your budget comfortably.`,
                  },
                  {
                    title: "Consider partial prepayment",
                    description:
                      "Even one extra EMI per year applied to the principal can reduce your overall tenure by 2–4 months and save meaningful interest.",
                  },
                  {
                    title: "Review refinancing if rate drops",
                    description: `If market rates drop more than 1% below your current ${calcRate}%, refinancing your loan could reduce your EMI and total interest burden.`,
                  },
                ]}
              />
            </DashboardSection>
