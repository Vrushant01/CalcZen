      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Loan term" value={years} onChange={setYears} suffix="yr" step={1} />
          </div>
          <Field label="Property tax (annual)" value={tax} onChange={setTax} prefix="$" />
          <Field label="Insurance (annual)" value={ins} onChange={setIns} prefix="$" />
          <Field label="HOA (monthly)" value={hoa} onChange={setHoa} prefix="$" />
          <Button variant="ghost" onClick={reset} size="sm">Reset</Button>
        </div>

        <div className="rounded-xl bg-muted/40 p-5 flex flex-col">
          <div className="text-sm text-muted-foreground">Estimated monthly payment</div>
          <div className="text-4xl font-bold mt-1 text-gradient">{fmt(result.monthly)}</div>
          <dl className="grid grid-cols-2 gap-3 mt-5 text-sm">
            <div><dt className="text-muted-foreground">Loan amount</dt><dd className="font-semibold">{fmt(result.principal)}</dd></div>
            <div><dt className="text-muted-foreground">Total interest</dt><dd className="font-semibold">{fmt(result.totalInterest)}</dd></div>
            <div><dt className="text-muted-foreground">Total paid</dt><dd className="font-semibold">{fmt(result.totalPaid)}</dd></div>
            <div><dt className="text-muted-foreground">P&I / month</dt><dd className="font-semibold">{fmt(result.pi)}</dd></div>
          </dl>
