...
export function PercentageCalculator() {
  ...
  const changeData = [
    { name: "Before", value: calcFrom, fill: "var(--color-chart-2)" },
    {
      name: "After",
      value: calcTo,
      fill: r3IsPositive ? "var(--color-chart-3)" : "var(--color-chart-5)",
    },
  ];
          ...
          >
            ...
            <DashboardSection title="Results">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <HeroMetric
                  ...
                  value={`${r3 >= 0 ? "+" : ""}${r3.toFixed(2)}%`}
                  badge={{
                    text: r3 >= 0 ? "Increase" : "Decrease",
                    color: r3 >= 0 ? "green" : "red",
                  }}
            ...
            <DashboardSection title="Derived Metrics">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <StatCard
                  index={1}
                  label={`Remainder (${100 - calcA}%)`}
                  value={(calcB - r1).toLocaleString()}
                  accent="default"
                />
                <StatCard
                  index={2}
                  label="Proportion ratio"
                  value={`${calcX}:${calcY - calcX}`}
                  accent="blue"
                  subValue={`${r2.toFixed(1)}% : ${(100 - r2).toFixed(1)}%`}
                />
                <StatCard
                  index={3}
                  label="Percent change"
                  value={`${r3 >= 0 ? "+" : ""}${r3.toFixed(2)}%`}
                  accent={r3IsPositive ? "green" : "red"}
                />
                <StatCard
                  index={4}
                  label="Absolute change"
                  value={`${r3IsPositive ? "+" : ""}${(calcTo - calcFrom).toLocaleString()}`}
                  accent={r3IsPositive ? "green" : "red"}
                />
                <StatCard
                  index={5}
                  label="Growth factor"
                  value={calcFrom !== 0 ? `${(calcTo / calcFrom).toFixed(3)}×` : "—"}
                  accent="amber"
                />
              </div>
            </DashboardSection>
            ...
            <DashboardSection title="Smart Insights">
              <div className="flex flex-col gap-2">
                ...
                <InsightCard
                  index={2}
                  tone={r3IsPositive ? "success" : "warning"}
                ...
                items={[
                  {
                    ...
                    description: `To find 20% off a $${calcB} item: ${calcA === 20 ? `${r1.toLocaleString()} off, final price ${(calcB - r1).toLocaleString()}` : `use 'X% of Y' with X=20 and Y=price`}. Always apply discounts sequentially, not additively.`,
                  },
                  {
                    title: "Tracking growth metrics",
                    description: `Use percent change for any metric that evolves over time — revenue, followers, weight, test scores. A ${r3 >= 0 ? "+" : ""}${r3.toFixed(1)}% change from ${calcFrom} to ${calcTo} can now be tracked consistently.`,
                  },
                  {
                    title: "Understanding percentages vs percentage points",
                    description:
                      "A rate going from 5% to 6% is a 1 percentage point increase, but a 20% relative increase. Always clarify which you mean in professional contexts.",
                  },
                ]}
              />
            </DashboardSection>
