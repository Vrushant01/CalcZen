...
export function WaterIntakeCalculator() {
  ...
  const comparisonRows = exerciseOptions.map((min) => {
    ...
    return {
      ...
      values: [
        `${totalL.toFixed(2)} L`,
        `${(totalL / 0.24).toFixed(1)} cups`,
        `${((baseMl + extraMl) / 1000 / 12).toFixed(2)} L/hr`,
      ],
      highlight: isActive,
    };
  });
  ...
  const pdfData = hasResult
    ? {
        ...
        results: [
          ...
          {
            label: "Equivalent Cups",
            value: `≈ ${cups.toFixed(1)} cups (240 ml)`,
            highlight: false,
          },
  ...
  const isButtonDisabled =
    !weightKg || weightKg <= 0 || activityMin === "" || Number(activityMin) < 0;
  ...
  const activeGauge =
    gaugeData.find((g) => liters <= g.threshold) ?? gaugeData[gaugeData.length - 1];
        ...
        {hasResult && (
          ...
          >
            ...
            <HeroMetric
              ...
              badge={{
                text: activeGauge.name,
                color:
                  activeGauge.name === "Good" || activeGauge.name === "Optimal"
                    ? "green"
                    : activeGauge.name === "Adequate"
                      ? "amber"
                      : "red",
              }}
            ...
            <DashboardSection title="Key Metrics">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <StatCard
                  ...
                  value={`${baseLiters.toFixed(2)} L`}
                  accent="blue"
                  subValue={`${calcWeightKg} kg × 35 ml`}
                />
                <StatCard
                  index={1}
                  label="Exercise Bonus"
                  value={`${exerciseLiters.toFixed(2)} L`}
                  accent="green"
                  subValue={`${calcActivityMin} min exercise`}
                />
                <StatCard
                  index={2}
                  label="Cups per Day"
                  value={`${cups.toFixed(1)} cups`}
                  accent="cyan"
                  subValue="240 ml per cup"
                />
                <StatCard
                  index={3}
                  ...
                  accent="purple"
                  subValue="~250 ml per glass"
                />
                <StatCard
                  index={4}
                  label="Per Hour (12 hrs)"
                  value={`${hourlyGoal} L/hr`}
                  accent="amber"
                  subValue="Spread evenly"
                />
                <StatCard
                  index={5}
                  label="ml per kg body"
                  value={`${mlPerKg.toFixed(0)} ml/kg`}
                  accent="default"
                />
              </div>
            </DashboardSection>
            ...
            <DashboardSection title="Hydration Gauge">
              <div className="rounded-xl border border-border/60 bg-card p-4 shadow-soft">
                <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">
                  Your intake vs hydration levels
                </div>
                <div className="h-44">
                      ...
                      <Bar dataKey="threshold" radius={[0, 4, 4, 0]}>
                        ...
                        {gaugeData.map((entry, i) => (
                          <Cell
                            key={i}
                            fill={entry.fill}
                            opacity={entry.name === activeGauge.name ? 1 : 0.3}
                          />
                        ))}
                      </Bar>
                    </BarChart>
            ...
            <DashboardSection title="Recommendations">
              <RecommendationList
                items={[
                  {
                    title: "Use urine color as your guide",
                    description:
                      "Pale straw yellow = well hydrated. Dark yellow = drink more immediately. Clear = possibly overhydrated. Check first thing in the morning for an accurate reading.",
                  },
                  {
                    title: "Drink before, during, and after exercise",
                    description: `With ${calcActivityMin} min of exercise, drink 300–500 ml before, 150–250 ml every 20 min during, and 500 ml after to fully replace sweat losses.`,
                  },
                  {
                    title: "Add 500ml on hot days",
                    description:
                      "Heat and humidity increase sweat rate significantly. On days above 30°C, add at least 500 ml to your daily target and consider electrolyte replacement for sessions over 60 min.",
                  },
                ]}
              />
            </DashboardSection>
