...
const badgeColors = {
  ...
  purple:
    "border-purple-300 bg-purple-50 text-purple-700 dark:border-purple-700 dark:bg-purple-950/60 dark:text-purple-300",
...
export function HeroMetric({ label, value, sub, badge, glow, className }: Props) {
  return (
    <div
      className={cn(
        "dashboard-hero-metric relative rounded-2xl border border-border/60 bg-card px-6 py-5 shadow-soft overflow-hidden",
        className,
      )}
    >
      {glow && (
      ...
      <div className="flex items-end gap-3 flex-wrap">
        <motion.div
          ...
          className="dashboard-hero-value text-gradient text-4xl font-extrabold leading-tight tracking-tight pb-1"
        >
          {value}
        </motion.div>
      ...
      {sub && <div className="dashboard-hero-sub mt-2 text-sm text-muted-foreground">{sub}</div>}
    </div>
  );
}
