...
export const Route = createFileRoute("/blog/$category/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    // Schema compiler: use database-generated structured data if present, otherwise build fallback graph
    const scripts = [];
    if (blog.jsonld) {
      ...
      // Dynamic fallback schema graphs
      scripts.push({
        type: "application/ld+json",
        ...
        children: JSON.stringify({
          ...
          itemListElement: [
            ...
            { "@type": "ListItem", position: 2, name: "Blog", item: `${siteUrl}/blog` },
            {
              ...
              position: 3,
              name: blog.category,
              item: `${siteUrl}/blog/${blog.category.toLowerCase()}`,
            },
            {
              "@type": "ListItem",
              position: 4,
    ...
    return {
      ...
      links: [
        ...
        ...(blog.thumbnail ? [{ rel: "preload", as: "image", href: blog.thumbnail, fetchpriority: "high" }] : []),
      ],
      scripts,
    };
  },
  component: BlogPageCategory,
  notFoundComponent: () => (
    <PageShell>
...
function BlogPageCategory() {
  ...
  const renderedContent = useMemo(() => {
    ...
    // Automatically inject links for high-value keyphrases inside paragraph tags
  ...
  // Sidebar Related Calculators retrieval
              ...
              {relatedBlogs.map((r) => (
                ...
                >
                  <Link
                    ...
                    params={{ category: r.category.toLowerCase(), slug: r.slug }}
